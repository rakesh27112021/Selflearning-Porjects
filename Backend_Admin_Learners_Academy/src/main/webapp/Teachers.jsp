<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
    
 <%@page import="java.util.ArrayList"%>
 <%@page import="com.dto.Teacher"%>
 <%@ include file = "Header.jsp"%>

  
 <% ArrayList teacherlist = (ArrayList)request.getAttribute("teacherlist");
 %>
 
 
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Teachers</title>

<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">

</head>

<body>

<div class="container col-md-1" style="overflow: auto;display: inline-block;">
</div>

<div class="container col-md-6" style="overflow: auto;display: inline-block;">
<h1>Teachers List</h1>
<table >
<tr>
<th class="col-md-6">Teacher name</th>
<th class="container col-md-6">Description</th>
<th></th>
</tr>
<%for(int i=0; i<teacherlist.size();i++){ %>

<% Teacher teacher =(Teacher)teacherlist.get(i); %>
<tr class='clickable-row'>
	<td>
	<%out.print(teacher.getTeacherName()); %>
	</td>
	<td>
	<%	out.print(teacher.getDescription()); %>
	</td>
	<td><form action="<%=request.getContextPath()%>/listteachers" method="post">
	<input style="display:none;" type="text" name="TeacherId"  value="<%out.print(teacher.getId());%>">
	<button type="submit" class="btn" name="deleteTeacher" value="deleteTeacher"><i class="fa fa-trash"></i></button>
	</form>
	</td>
	
</tr>

<% } %>


</table>

</div>

<div class="container col-md-3" style="overflow: auto;display: inline-block; position: absolute">
  <h1>Add New Class</h1>
  <form action="<%=request.getContextPath()%>/listteachers" method="post">
   <div class="form-group">
    <label for="name">Teacher Name: </label> <input type="text"
     class="form-control" id="name" placeholder="Teacher Name"
     name="name" required>
   </div>
   <div class="form-group">
    <label for="name">Discription: </label> <input type="text"
     class="form-control" id="discription" placeholder="Discription"
     name="discription">
   </div>
   <button type="submit" class="btn btn-primary" name="addTeacher" value="addTeacher">Add</button>
  </form>
  
 </div>
 


</body>

</html>